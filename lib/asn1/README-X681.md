...
To use this feature use the `--one-code-file` argument to `asn1_compile`.
Both the `--template` and codegen backends support automatic open type
handling.  C types are generated from ASN.1 types as described above.
...
NOTE: This has already be implemented in the `master` branch of Heimdal.
 ...
 - Both backends now support automatic open type decoding/encoding.  The
   codegen backend generates explicit if/else chains for type-ID matching
   and per-object decode/encode/free/copy code, while the template backend
   uses data-driven tables at runtime.  The template compiler is still
   superior on output size scaling as `O(N)` instead of `O(M * N)` where `M`
   is the number of encoding rules supported and `N` is the size of an ASN.1
   module (or all modules), but the codegen backend may be useful for
   generating code for languages that aren't dynamic enough to support the
   templating approach.
